    ...
    class Performance
      ...
      class Options
        ...
        def initialize(raw_options = {})
          ...
          @include_inactive_practitioner = raw_options[:include_inactive_practitioner]
          @invoice_stats_by_service_date = raw_options[:invoice_stats_by_service_date]
        end
        ...
        def to_params
          ...
          if include_inactive_practitioner.present?
            params[:include_inactive_practitioner] = 1
          end

          if invoice_stats_by_service_date.present?
            params[:invoice_stats_by_service_date] = 1
          end
      ...
      def as_csv
        CSV.generate(headers: true) do |csv|
          ...
          report_start_date_cell = options.start_date.strftime('%Y-%m-%d')
          report_end_date_cell = options.end_date.strftime('%Y-%m-%d')

          @result[:data].each do |row|
            pract = row[:practitioner]
            pract_group_col = pract.groups.to_a.map(&:name).join(';')
            csv << [
              report_start_date_cell,
              report_end_date_cell,
              pract.id,
              pract.full_name,
              pract.profession,
              pract.state,
              ...
              (row['appointments_count'].zero? || row['patients_count'].zero?) ? 0 : (row['appointments_count'].to_f / row['patients_count'].to_f).round(1),
              row['treatment_notes_count'],
              row['letters_count'],

              (row['availability_duration'].to_f / 3600).round(2),
              (row['appointments_duration'].to_f / 3600).round(2),
              (row['travel_distance'].to_f / 1000).round(2),
      ...
      def calculate
        @result = {}
        invoices_aggregate_statements =
          if options.invoice_stats_by_service_date
            "
            COALESCE((
              SELECT
                SUM(invoices.amount)
              ...
              WHERE
                ...
                AND invoices.service_date >= :start_date
                AND invoices.service_date <= :end_date
              GROUP BY PRAC.id
            ...
            COALESCE((
              ...
              WHERE
                invoices.practitioner_id = PRAC.id
                AND invoices.deleted_at IS NULL
                AND invoices.issue_date >= :start_date
                AND invoices.issue_date <= :end_date
              GROUP BY PRAC.id
            ), 0) AS invoiced_amount,
        ...
        query = "
          SELECT
            ...
            COALESCE(SUM((AVAI.cached_stats->'travel_distance')::integer), 0) AS travel_distance,
      ...
      def report_practitioner_ids
        @report_practitioner_ids ||= begin
          ...
          elsif !options.include_inactive_practitioner
            query = query.active
          end
