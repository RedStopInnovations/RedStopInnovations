  ...
  module Dva
    class PaymentAvailability
      ...
      def validate_patient(patient)
        ...
        if patient.dva_details.blank?
          error = 'The client\'s DVA details is missing.'
        end

        if error.blank? && patient.dva_file_number.blank?
          error = 'The client\'s DVA file number is missing.'
        end
      ...
      def validate_invoice(invoice)
        ...
        if error.blank?
          has_non_claimable_item = invoice.items.billable_item.any? do |invoice_item|
            !invoice_item.invoiceable.is_dva_item?
          end

          if has_non_claimable_item
            error = 'The invoice contains non-claimable items for DVA.'
          end
        end
