module Dashboard
  # Overview report for admin users
  class BusinessOverviewReport
    STATS_RANGES = [
      'current_month',
      'last_30_days',
      'last_60_days',
    ...
    attr_reader :business, :current_date
    ...
    def calculate_appointment_stats
      ...
      STATS_RANGES.each do |range|
        ...
        appointments_query = business.appointments.
        ...
        referrals_count = business.referrals.where(created_at: time_range).count

        availability_query = business.availabilities.
        ...
        cancellations_count = business.appointments.
          where(start_time: time_range).
        ...
        total_driving_distance = (availability_query.sum("(cached_stats->'travel_distance')::integer").to_f / 1000).round(2)
        total_driving_duration = (availability_query.sum("(cached_stats->'travel_duration')::integer").to_f / 3600).round(2)
        total_availability_time = (availability_query.sum("(cached_stats->'availability_duration')::integer").to_f / 3600).round(1)
    ...
    def calculate_transaction_stats
      ...
      STATS_RANGES.each do |range|
        ...
        total_payment_amount = business.payments.where(created_at: time_range).where(deleted_at: nil).sum(:amount).round(2)
