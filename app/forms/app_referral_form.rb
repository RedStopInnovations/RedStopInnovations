class AppReferralForm < BaseForm
  ...
  validate do
    ...
    attachments.each do |attm|
      if attm.present? && !(attm.is_a?(ActionDispatch::Http::UploadedFile) &&
  ...
  # Create form instance from a referral record
  # @param ::Referral referral
  def self.hydrate(referral)
    attrs = {
      type: referral.type.presence,
      business: referral.business
    }

    %i(
      availability_type_id professions practitioner_id medical_note priority referral_reason
      referrer_business_name referrer_name referrer_phone referrer_email
    ).each do |attr|
      attrs[attr] = referral[attr].presence
    end

    # Patient info
    patient_info = PatientInfo.new

    patient_info.attributes.keys.each do |attr|
      patient_info[attr] = referral.patient_attrs[attr]
    end

    patient_info.referral_type = attrs[:type]

    attrs[:patient] = patient_info
