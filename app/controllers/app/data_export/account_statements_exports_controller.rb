module App
  module DataExport
    class AccountStatementsExportsController < ApplicationController
      ...
      def index
        ahoy_track_once 'Use account statements export'

        @exports = AccountStatementsExport.where(business_id: current_business.id).order(id: :desc).page(params[:page])
      end

      def create
        ahoy_track_once 'Use account statements export'

        @form = CreateAccountStatementsExportForm.new(create_account_statement_export_params.merge(
        ...
        if @form.valid?
          ...
          unless reject_error
            total_exports_24h = AccountStatementsExport
          ...
          unless reject_error
            last_pending_export_24h = AccountStatementsExport
              .where(business_id: current_business.id)
          ...
          unless reject_error
            pending_export_24h_count = AccountStatementsExport
              .where(business_id: current_business.id)
              .where('created_at >= ?', 24.hours.ago)
              .pending
              .count
          ...
          if reject_error
            ...
            CreateAccountStatementsExportService.new.call(current_user, @form)
            flash[:notice] = 'The export progress is started.'
      ...
      def show
        ...
        @export = AccountStatementsExport.where(business_id: current_business.id).find(params[:id])
      end
      ...
      def create_account_statement_export_params
        params.permit(
          :start_date,
          :end_date,
          :payment_status,
